...
public class AnalysisMapper extends TableMapper<Text, Text> {
    ...
    protected void map(ImmutableBytesWritable key, Result value, Context context) throws IOException, InterruptedException {
        ...
        //133     -     201806,     1    -  50min
        //主叫,年度统计
        context.write(new Text(call1 + "-" +year),durationText);
        //主叫,月度统计
        context.write(new Text(call1 + "-" +month),durationText);
        //主叫,日度统计
        context.write(new Text(call1 + "-" +day),durationText);

        //被叫,年度统计
        context.write(new Text(call2 + "-" +year),durationText);
        //被叫,月度统计
        context.write(new Text(call2 + "-" +month),durationText);
        //被叫,日度统计
        context.write(new Text(call2 + "-" +day),durationText);
